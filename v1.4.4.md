    ...
    visibleRows.forEach(row => {
        ...
        if (year && !isNaN(year)) {
            ...
            FEATURE_FIELDS.forEach(field => {
                // Use papersStore.fieldIsTrue for text_presence awareness
                if (papersStore.fieldIsTrue(c, field)) yearlyFeatures[year][field]++;
            });
        }
    });
...
/* Skip rendering work for off-screen paper rows.
   contain-intrinsic-size gives the browser a height estimate
   so the scrollbar stays accurate before the row is laid out. */
#papersTable tbody tr[data-paper-id] {
    content-visibility: auto;
    contain-intrinsic-size: auto 58px;   /* ≈ one row height */
}
```
  ...
  {% for paper in papers %}
    ...
    <tr data-paper-id="{{ paper.id }}">
      ...
      <td class="status-cell pdf-status emoji-column">
        {% if paper.pdf_filename and paper.pdf_state in ['PDF', 'annotated'] %}
...
# web\templates\shared\paper_cells.html
```html
{#<!-- shared/paper_cells.html -->#}
{% macro render_domain_cells(c, certainty, domain_config, editable=true) %}
{% for group in domain_config.groups %}
  {% if group.filter_type == 'tri_state' %}
    {% set path = group.json_path %}
    <td class="status-cell {{ 'editable-status' if editable }} certainty-{{ certainty.get(path, 'solid') }}" data-field="{{ path }}">
  ...
  {% elif group.filter_type in ['inclusion', 'none'] %}
    {% for field_def in group.fields %}
      ...
      {% else %}
        <td class="status-cell {{ 'editable-status' if editable }} {{ highlight_class }} certainty-{{ certainty.get(path, 'solid') }}" data-field="{{ path }}">
          {% if certainty.get(path) == 'conflict' %}<span class="conflict-warning">⚠️</span>
          {% else %}<span class="emoji-content">{{ c | get_json_path(path) | render_status }}</span>{% endif %}
        </td>
      {% endif %}
...
<tbody style="position: relative;">
    ...
    <tr data-paper-id="{{ paper.id }}">
      ...
      <td class="status-cell pdf-status emoji-column">
        ...
        {% endif %}
        {#<!--  pdf_state == 'none': render nothing. No ❔ in exports. -->#}
      </td>
      ...
      <td class="hidden-data-cell" data-field="abstract" style="display: none;">
        {% if skip_abstracts %}
          No data.
        {% else %}
          {{ paper.abstract }}
      ...
      <td colspan="{{ total_cols }}">
        <div class="detail-flex-container">
          <div class="detail-content detail-abstract">
            ...
            {% if skip_abstracts %}
              <em>Abstract removed to avoid potential copyright complaints.</em>
            {% else %}
              {{ paper.abstract }}
            {% endif %}
          </p>
